...
let is_active_arg pat =
  List.exists (fun {attr_name} -> attr_name.txt = "active") pat.pat_attributes
  ...
  let two_or_more_mentions tactic var_name expr =
    let rec two_or_more_mentions expr count =
      ...
      | Texp_match (e, cs,  _) ->
        ...
        let exprs = cs |> List.filter_map (fun c ->
          let vars = get_pat_vars c.c_lhs in
          if List.for_all ((<>) var_name) @@ vars
          then Some c.c_rhs
          else None
          )
        in
...
let rec create_fresh_argument_names_by_type (typ : Types.type_expr) =
  ...
  and translate_rec_let let_vars bind expr =
    ...
    let has_tabled_attr = Util.has_named_attr tabling_attr_name bind.vb_attributes in
...
let add_packages ast =
  List.map (fun n -> Str.open_ @@ Opn.mk (Mod.ident @@ mknoloc @@ Lident n)) packages @ ast
...
let fresh_and_conjs_normalizer need_move_unifies_up =
  ...
  let has_heavy_attr e = Util.has_named_attr "heavy" e.pexp_attributes in
...
let call_by_need_creator tree =
  ...
  let has_attr_for_expr attr expr = Util.has_named_attr attr expr.pexp_attributes in
  ...
  let vb_is_logic vb = Util.has_named_attr "need_CbN" vb.pvb_attributes in

  let expr_is_logic e = Util.has_named_attr "need_CbN" e.pexp_attributes in
  ...
  let var_is_logic pat = Util.has_named_attr "logic" pat.ppat_attributes in

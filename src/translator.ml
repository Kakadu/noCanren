        ...
        let get_pat_vars : type a. a Typedtree.general_pattern -> string list =
         fun pat ->
          ...
          parse
            ...
            pat.pat_loc
            pat
            ~on_error:(fun _str -> failwith "Not implemented")
            (fun xs () -> xs)
            ()
        in
        match expr.exp_desc with
    ...
    fun (type a) loc translated_scrutinee attrs (cases : a Typedtree.case list) typ ->
     ...
     then (
       ...
       let translate_case case =
         ...
         in
         let abst_body =
           List.fold_right create_fun (List.map (fun v -> v ^ "_o") vars) body
         in
...
let only_generate tast params =
  try
    ...
    |> eval_if_need
         ...
         (let mapper = fresh_and_conjs_normalizer params.move_unifications in

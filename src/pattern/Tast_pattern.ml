  ...
  T
    (let rec helper : type a. _ -> _ -> a general_pattern -> _ =
      fun ctx loc x k ->
       ...
       | _ -> fail loc "tpat_constant"
     in
     helper)
;;

let tpat_var (T ident) =
  T
    (let rec helper : type a. _ -> _ -> a general_pattern -> _ =
      fun ctx loc x k ->
       match x.pat_desc with
    ...
    (let rec helper : type a. _ -> _ -> a general_pattern -> _ =
      fun ctx loc x k ->
       ...
       | Tpat_construct (_, _, xs) ->
         ctx.matched <- ctx.matched + 1;
         k |> ident ctx loc xs
       | _ -> fail loc "tpat_construct"
     in
     helper)
;;

let tpat_record (T ident) =
  T
    (let rec helper : type a. _ -> _ -> a general_pattern -> _ =
      fun ctx loc x k ->
       match x.pat_desc with
...
let fix : 'a 'b 'c. (unit -> ('a, 'b, 'c) t -> ('a, 'b, 'c) t) -> unit -> ('a, 'b, 'c) t =
 fun f ->
  let self = ref (fun () -> T (fun _ -> assert false)) in
  let ans () = f () (!self ()) in
  self := ans;
  ans
;;
